...
namespace FlowOps.Services.Reporting.Sql
{
    public sealed class SqlReportingProjector : IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("SqlReportingProjector started (subscribed to reporting events).");
        ...
        private async Task OnCancelled(SubscriptionCancelledEvent ev)
        {
            ...
            await ExecAsync(connection,
                "UPDATE dbo.CustomerReports " +
                "SET ActiveSubscriptions = CASE WHEN ActiveSubscriptions > 0 THEN ActiveSubscriptions - 1 ELSE 0 END " +
        ...
        private async Task EnsureCustomerRowAsync(Guid customerId, CancellationToken ct = default)
        {
            ...
            await ExecAsync(connection,
                ...
                "INSERT INTO dbo.CustomerReports(CustomerId, ActiveSubscriptions, TotalInvoiced, TotalPaid) VALUES(@cid, 0, 0, 0);",
                p => p.Add(new SqlParameter("@cid", SqlDbType.UniqueIdentifier) { Value = customerId }),
                ct);
        }
